  ...
  try {
    ...
    if (contactName) {
      try {
        ...
        logger.warn({ error: err, hashToUse }, 'Failed to backfill request_history with contactName');
      }
    }
...
export const updateContact = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (previous && previous.phoneNumberHash && updated.phoneNumberHash && previous.phoneNumberHash !== updated.phoneNumberHash) {
      try {
        await requestHistoryRepository.updatePhoneNumberHash(previous.phoneNumberHash, updated.phoneNumberHash);
        logger.info(
          { oldHash: previous.phoneNumberHash.substring(0, 8), newHash: updated.phoneNumberHash.substring(0, 8) },
          'Updated phone hash in request_history'
        );
      } catch (err) {
        logger.warn({ error: err, previousHash: previous.phoneNumberHash, newHash: updated.phoneNumberHash }, 'Failed to update phone hash in request_history');
      }
    }
    ...
    if (updated.phoneNumberHash && updated.contactName) {
      try {
        await requestHistoryRepository.updateContactNameForPhone(updated.phoneNumberHash, updated.contactName, true);
      } catch (err) {
        logger.warn({ error: err, contactId: updated.id }, 'Failed to backfill request_history after update');
      }
    }
    try {
      webSocketService.emit(SocketEvents.REQUEST_CONTACT_UPDATE, {
      ...
      if (previous && previous.phoneNumberHash && previous.phoneNumberHash !== updated.phoneNumberHash) {
        webSocketService.emit(SocketEvents.REQUEST_CONTACT_UPDATE, {
    ...
    try {
      ...
      if (contact?.phoneNumberHash) {
        try {
          ...
          logger.warn({ error: err, phoneHash: contact.phoneNumberHash }, 'Failed to clear request_history contact_name on delete');
        }
      }
      webSocketService.emit(SocketEvents.REQUEST_CONTACT_UPDATE, {
