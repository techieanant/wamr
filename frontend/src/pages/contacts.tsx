...
export default function ContactsPage() {
  ...
  const filteredContacts = contacts.filter((c) => {
    if (query) {
      ...
      const phoneCandidate = (c.phoneNumber || c.maskedPhone || c.phoneNumberHash || '').toLowerCase();
      const nameCandidate = (c.contactName || '').toLowerCase();
      if (!phoneCandidate.includes(q) && !nameCandidate.includes(q) && !String(c.id).includes(q)) return false;
    }
    return true;
  });
                ...
                <TableBody>
                  {sortedContacts.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell className="font-mono text-sm" onClick={() => !editingId && startEdit(c.id, c.contactName, c.phoneNumber)}>
                        {editingId === c.id ? (
                          <Input
                            ref={inputRef}
                            value={editingPhone}
                      ...
                      <TableCell onClick={() => !editingId && startEdit(c.id, c.contactName, c.phoneNumber)}>
                        {editingId === c.id ? (
                          <Input
                            value={editingName}
                      ...
                      <TableCell className="text-right">
                        {editingId === c.id ? (
                          ...
                          <div className="flex justify-end gap-2">
                            <Button variant="outline" size="sm" onClick={() => startEdit(c.id, c.contactName, c.phoneNumber)}>
